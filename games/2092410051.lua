            ...
            if CollectPresents:IsEnabeld() then
                for _, Present in pairs(workspace.Map.Holiday.Items:getChildren()) do
                    if Present.Name == "Present" and Present.Transparency == 0 then
                        ...
                        wait(0.1)
                        if CollectPresents:IsEnabeld() == false then
                            break
                        end
                    end
                end
                    ...
                    for _, Descendant in pairs(Plot.Objects:GetDescendants()) do
                        if Descendant.Name == "Shelves" and Descendant:FindFirstChild("Base") then
                            ...
                            if #Amount == 2 then
                                ...
                                if buyAmount ~= 0 then
                                    print("Buy " .. tostring(buyAmount) .. ' x "' ..
                                              Sellables[Descendant.Sellable.Value] .. '"')
                                    ReplicatedStorage.Remotes.BuyStorage:InvokeServer(
                                        Sellables[Descendant.Sellable.Value], buyAmount, false)
                                end
        ...
        while wait() do
            if AutoDelivery:IsEnabeld() then
                ...
                if Car ~= nil and Plot ~= nil then
                    ...
                    if getrenv()._G.Data.DockCurrentStorage > 0 and getrenv()._G.Data.VehicleMaxStorage ~=
                        getrenv()._G.Data.VehicleCurrentStorage then
                        -- Tp to Loading Dock
                        TeleportCar(GetLoadingDock().ParkingArea.Floor.CFrame * CFrame.Angles(0, math.rad(-90), 0) +
                                        Vector3.new(0, 5, 0))
                        wait(1)

                        -- LoadVehicle
                        local Storage, MaxStorage, CurrentStorage, VehicleStorage, VehicleMaxStorage,
                            VehicleCurrentStorage, DockStorage, DockCurrentStorage = ReplicatedStorage.Remotes
                                                                                         .LoadVehicle:InvokeServer()
                    ...
                    if getrenv()._G.Data.VehicleCurrentStorage > 0 then
                        ...
                        for _, Descendant in pairs(Plot.Walls:GetDescendants()) do
                            if string.find(Descendant.Name, "Door") then
                                if Descendant:FindFirstChild("Base") and Descendant:FindFirstChild("Handle") then
                                    ...
                                    TeleportCar(Descendant.Parent.Base.CFrame * CFrame.Angles(0, math.rad(180), 0) +
                                                    Vector3.new(1, 2, 1))
                                    break
                                end
                            end
                        ...
                        if getrenv()._G.Data.DockCurrentStorage <= 0 and getrenv()._G.Data.VehicleCurrentStorage <= 0 then
                            -- Waiting Spot (Plot)
                            TeleportCar(Plot.RoadModule.Center.CFrame * CFrame.Angles(0, math.rad(180), 0) +
                                            Vector3.new(0, 10, 0))
                        end
                    end
                end
...
Teleports:addButton("Loading Dock", function()
    LocalPlayer.Character.PrimaryPart.CFrame = workspace.Map.Landmarks["Loading Dock"].MicrokSpawn.CFrame +
                                                   Vector3.new(-5, 0, 0)
end)
